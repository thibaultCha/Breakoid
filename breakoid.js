...
;(function (window) {
    ...
    function Breakoid (canvasId) {
        ...
        , _ballX = 100, _ballY = 250, _ballDirX = 1, _ballDirY = -1, _ballSpeed = 5 // ball vars
        , _interval // interval
        ...
        function ballOnTick () {
            var nextX = _ballX + _ballDirX * _ballSpeed
            var nextY = _ballY + _ballDirY * _ballSpeed
            if (nextX + Breakoid.BALL_SIZE > _gameWidth) _ballDirX  = -1 // right border
            else if (nextX < 0 + Breakoid.BALL_SIZE)     _ballDirX  = 1 // left border
            if (nextY + Breakoid.BALL_SIZE > _gameHeight) _ballDirY = -1 // game is lost gameLost()
            else {
                if (nextY < 0 + Breakoid.BALL_SIZE) _ballDirY = 1 // top border
                else {
                    // Ball touches the bar
                    if ( 
                        nextY > _gameHeight - Breakoid.BAR_HEIGHT - Breakoid.BALL_SIZE
                        && (nextX >= _barX && nextX <= _barX + Breakoid.BAR_WIDTH)
                        ) {
                        _ballDirY = -1
